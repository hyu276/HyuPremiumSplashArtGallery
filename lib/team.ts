...
export async function getTeamMembers(): Promise<TeamMember[]> {
  if ((backendCatalogue as any).ready === true) {
    return (backendTeam as any[]).filter(row=>!row?.hidden).map(row=>({
      id:Number(row.id)||0,name:String(row.name||''),image:String(row.image||''),sort_order:Number(row.sort_order)||0,
      facebook_url:String(row.facebook_url||''),facebook_hidden:Boolean(row.facebook_hidden),tiktok_url:String(row.tiktok_url||''),tiktok_hidden:Boolean(row.tiktok_hidden),instagram_url:String(row.instagram_url||''),instagram_hidden:Boolean(row.instagram_hidden),x_url:String(row.x_url||''),x_hidden:Boolean(row.x_hidden),linkedin_url:String(row.linkedin_url||''),linkedin_hidden:Boolean(row.linkedin_hidden)
  ...
  const response=await fetch(`${SUPABASE_URL}/rest/v1/team_members?select=${encodeURIComponent(select)}&hidden=eq.false&order=sort_order.asc,id.asc`,{
    headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${SUPABASE_KEY}`},next:{revalidate:300,tags:['team']}
  });
  if(!response.ok)return [];
  return response.json() as Promise<TeamMember[]>;
}

export function safeSocialUrl(value:string){try{const url=new URL(String(value||''));return /^https?:$/.test(url.protocol)?url.href:''}catch{return ''}}
