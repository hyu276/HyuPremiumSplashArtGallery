...
type Catalogue={schemaVersion?:number;generatedAt?:string;items:any[];categories:string[];ranks:string[];credits:string[];skinlines?:string[];universes?:UniverseDef[];taxonomyRepresentatives?:Partial<TaxonomyRepresentatives>;ownerOptions?:OwnerOptions;championThumbnails?:Record<string,ChampionThumbnailChoice>};
type AdminPayload={ownerItems?:any[];categories?:string[];ranks?:string[];credits?:string[];skinlines?:string[];universes?:UniverseDef[];taxonomyRepresentatives?:Partial<TaxonomyRepresentatives>;championThumbnails?:Record<string,ChampionThumbnailChoice>;team?:any[];seo?:any};
...
function itemSkinline(item:any){if(item&&Object.prototype.hasOwnProperty.call(item,'skinline'))return String(item.skinline||'').trim();return Array.isArray(item?.skinlines)?String(item.skinlines[0]||'').trim():''}
...
function normalizeTaxonomyRepresentatives(value:unknown,items:any[],skinlines:string[],universes:UniverseDef[]):TaxonomyRepresentatives{const raw=value&&typeof value==='object'&&!Array.isArray(value)?value as any:{};const result:TaxonomyRepresentatives={skinlines:{},universes:{}};for(const skinline of skinlines){const id=String(raw?.skinlines?.[skinline]||'').trim();if(id&&items.some(item=>String(item.id)===id&&String(itemSkinline(item)).toLowerCase()===skinline.toLowerCase()))result.skinlines[skinline]=id}for(const universe of universes){const id=String(raw?.universes?.[universe.name]||'').trim();if(id&&items.some(item=>String(item.id)===id&&universe.skinlines.some(line=>line.toLowerCase()===itemSkinline(item).toLowerCase())))result.universes[universe.name]=id}return result}
...
function sameArtworkMediaLineage(next:any,current:any){if(!current||String(next?.image||'')!==String(current?.image||''))return false;return VARIANT_WIDTHS.every(width=>{const key=String(width);const nextUrl=String(next?.variants?.[key]?.url||'');const currentUrl=String(current?.variants?.[key]?.url||'');return !nextUrl||nextUrl===currentUrl})}
function preserveArtworkMedia(item:any,currentById:Map<string,any>){const next=canonicalItem(item);const prior=currentById.get(String(next.id||''));if(!prior||!sameArtworkMediaLineage(next,prior))return next;if(!next.expanded&&prior.expanded)next.expanded=prior.expanded;if(!completeVariants(next)&&completeVariants(prior))next.variants=prior.variants;return next}
...
export async function POST(request:Request){
  ...
  try{
    ...
    if(catalogueRequested){
      ...
      const preferredSkinlines=orderedUnique((Array.isArray(payload.skinlines)?payload.skinlines:(current.skinlines||[])).map(String));
      const preferredUniverses=normalizeUniverses(payload.universes!==undefined?payload.universes:(current.universes||[]),preferredSkinlines);
      const currentById=new Map((current.items||[]).map(item=>[String(item?.id||''),item]));const requested=(Array.isArray(payload.ownerItems)?payload.ownerItems:current.items||[]).map(canonicalItem);const requestedTaxonomy=requested.map(item=>taxonomyItem(item,preferredSkinlines,preferredUniverses)).map(item=>preserveArtworkMedia(item,currentById));const preferredRepresentatives=normalizeTaxonomyRepresentatives(payload.taxonomyRepresentatives!==undefined?payload.taxonomyRepresentatives:current.taxonomyRepresentatives,requestedTaxonomy,preferredSkinlines,preferredUniverses);
      ...
      const invalidSkinlines=unique(requested.map(itemSkinline).filter(value=>Boolean(value)&&!preferredSkinlines.some(line=>line.toLowerCase()===value.toLowerCase())));
      if(invalidCategories.length||invalidCredits.length||invalidRanks.length||invalidSkinlines.length){
        return Response.json({error:'Taxonomy không hợp lệ: tác phẩm đang tham chiếu tùy chọn không còn tồn tại. Hãy cập nhật tác phẩm trước khi publish.',requestId,invalid:{categories:invalidCategories,credits:invalidCredits,ranks:invalidRanks,skinlines:invalidSkinlines}},{status:409,headers:responseHeaders(request,requestId)});
      }
      const enriched=[];for(const item of requestedTaxonomy)enriched.push(await enrichMedia(item,storageBase,token));
      const missingExpanded=enriched.filter(item=>!String(item?.expanded?.url||'').trim()||!Number(item?.expanded?.width)||!Number(item?.expanded?.height)||!Number(item?.expanded?.bytes));if(missingExpanded.length)throw new Error(`Không thể publish catalogue: thiếu expanded display asset hợp lệ cho ${missingExpanded.map(item=>String(item?.id||'<unknown>')).join(', ')}.`);
      ...
      const catalogue:Catalogue={...current,schemaVersion:4,generatedAt:new Date().toISOString(),items,categories,ranks,credits,skinlines:preferredSkinlines,universes:preferredUniverses,taxonomyRepresentatives:preferredRepresentatives,ownerOptions,championThumbnails};
    ...
    if(!result.noop){
      ...
      if(catalogueRequested){revalidateTag('catalogue');for(const path of ['/','/character/','/artworks/','/champions/','/skinlines/','/universes/','/sitemap.xml','/image-sitemap.xml'])paths.add(path)}
