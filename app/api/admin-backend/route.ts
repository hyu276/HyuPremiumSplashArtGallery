...
function itemSkinline(item:any){const direct=String(item?.skinline||'').trim();if(direct)return direct;return Array.isArray(item?.skinlines)?String(item.skinlines[0]||'').trim():''}
