import { artworkPath, artworkPreview, getCatalogue, siteUrl } from '@/lib/catalogue';
...
export async function GET(){
  ...
  const body=`<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1">${items.map(item=>{const path=artworkPath(item),override=map.get(path);if(override?.include_in_sitemap===false)return '';const loc=override?.canonical_url||`${siteUrl}${path}`,image=artworkPreview(item,1600),title=override?.title||item.name,caption=override?.image_alt||`${item.name} — ${item.category} gaming splash art`;return `\n<url><loc>${xml(loc)}</loc><image:image><image:loc>${xml(image)}</image:loc><image:title>${xml(title)}</image:title><image:caption>${xml(caption)}</image:caption></image:image></url>`}).join('')}\n</urlset>`;
